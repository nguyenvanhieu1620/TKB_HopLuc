...
import { getClassMajorTrainingMode } from "./trainingModeCheck";
...
export async function runAutoScheduleExams(classId: number, semesterId: number, userId: number): Promise<AutoExamScheduleReport> {
  ...
  // Việc CC: rải đều — mỗi ngày tối đa 1 ca thi/Lớp, trừ khi số môn cần thi nhiều hơn số ngày còn lại
  // trong vùng thi thì mới cho phép nhiều hơn 1 ca/ngày (chia đều tối thiểu cần thiết).
  const maxExamsPerDay = pendingSubjects.length > examDates.length
    ? Math.ceil(pendingSubjects.length / examDates.length)
    : 1;
